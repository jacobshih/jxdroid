...
public class TW319Village extends TW319Location implements Serializable {
	...
	private void getStoresOnLine() {
		...
		try {
			...
			for (Element e : elems) {
				/*
				 * tds contains information of the store, as described below, 0,
				 * the category icon (<img src="..." title="...">). 1, name and
				 * link of the store (<a href="...">). 2, telephone of the
				 * store. 3, address of the store.
				 */
				...
				TW319StoreItem item = new TW319StoreItem(this, storeId, storeName,
						storeUrl, storeCategory, storeIcon, storeTelephone,
						storeAddress);
				addLocationItem(item);
			}
			saveStores();
